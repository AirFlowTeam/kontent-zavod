...
export async function refreshAccess(platform, credentials, options = {}) {
  ...
  if (platform === 'Instagram') {
    ...
    r = await json('https://id.vk.ru/oauth2/auth', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'refresh_token', refresh_token: credentials.refreshToken, client_id: credentials.clientId, device_id: credentials.deviceId, state }) }, options);
  } else return null;
