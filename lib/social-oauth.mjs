import { SocialApiError, parseCredentials, inspectYouTubeOAuthAccount, youtubeReadonlyScope } from './social-api.mjs';

// Server configuration only. Never return app secrets or tokens to browser DTOs.
export const oauthProviders = {
  youtube: { platform: 'YouTube', prefix: 'YOUTUBE', required: ['YOUTUBE_CLIENT_ID', 'YOUTUBE_CLIENT_SECRET'], scopes: [youtubeReadonlyScope], auth: 'https://accounts.google.com/o/oauth2/v2/auth' },
...
export function integrationStatus(env) {
  ...
  return [...Object.entries(oauthProviders).map(([id, p]) => {
    ...
    return { id, platform: p.platform, ready: missing.length === 0 && enabled, missing, sharedKeyConfigured: false,
  ...
  { id: 'rutube', platform: 'RuTube', ready: true, missing: [], sharedKeyConfigured: false, status: 'Публичный сбор без ключа; лайки недоступны', callbackUrl: null, scopes: [], enableVariable: null }];
}
...
export async function authorizationUrl(id, env, state, verifier) {
  ...
  if (id === 'youtube' && !/^[A-Za-z0-9._~-]{43,128}$/.test(verifier || '')) throw new SocialApiError('Начните вход через Google заново.');
  const u = new URL(p.auth);
  u.search = new URLSearchParams({ response_type: 'code', redirect_uri: p.redirectUri, scope: p.scopes.join(['vk', 'youtube'].includes(id) ? ' ' : ','), state,
    ...(id === 'tiktok' ? { client_key: env.TIKTOK_CLIENT_KEY } : { client_id: env[`${p.prefix}_CLIENT_ID`] }),
    ...(id === 'instagram' ? { force_reauth: 'true' } : {}),
    ...(id === 'youtube' ? { access_type: 'offline', prompt: 'consent select_account' } : {}),
    ...(['vk', 'youtube'].includes(id) ? { code_challenge: await pkceChallenge(verifier), code_challenge_method: 'S256' } : {}) }).toString();
  return u.toString();
}
...
export function callbackParams(id, url) {
  ...
  for (const k of keys) {
    ...
    if (value !== null && value !== undefined) {
      if (value.length > 8192 || [...value].some((character) => character.charCodeAt(0) < 32)) throw new SocialApiError('Некорректный ответ авторизации.');
      result[k] = value;
    }
  }
...
const allowedOrigins = new Set(['https://api.instagram.com', 'https://graph.instagram.com', 'https://graph.threads.com', 'https://open.tiktokapis.com', 'https://id.vk.ru', 'https://oauth2.googleapis.com']);
async function tokenRequest(url, init, options) {
  ...
  try {
    const r = await (options.fetchImpl || fetch)(url, { ...init, redirect: 'manual', signal: AbortSignal.timeout(20000) });
    if (r.status >= 300 && r.status < 400) { await r.body?.cancel(); throw new Error(); }
...
export async function exchangeCode(id, env, params, verifier, options = {}) {
  ...
  let r; let accountId;
  if (id === 'youtube') {
    if (!/^[A-Za-z0-9._~-]{43,128}$/.test(verifier || '')) throw new SocialApiError('Начните вход через Google заново.');
    r = await tokenRequest('https://oauth2.googleapis.com/token', {
      method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ client_id: env.YOUTUBE_CLIENT_ID, client_secret: env.YOUTUBE_CLIENT_SECRET, code_verifier: verifier, grant_type: 'authorization_code', redirect_uri: p.redirectUri, code: params.code }),
    }, options);
    requireScopes(r.scope, p.scopes);
    if (String(r.token_type).toLowerCase() !== 'bearer') throw new SocialApiError('Google не подтвердил тип доступа. Войдите через Google заново.');
    if (!r.refresh_token) throw new SocialApiError('Google не выдал доступ для ежедневного сбора. Войдите через Google заново и подтвердите разрешение.');
    accountId = await inspectYouTubeOAuthAccount(parseCredentials({ accessToken: r.access_token, refreshToken: r.refresh_token, authType: 'youtube_oauth' }), options);
  } else if (id === 'threads') {
  ...
  return { accountId, credentials: parseCredentials({ accessToken: r.access_token, refreshToken: r.refresh_token,
    ...(id === 'youtube' ? { authType: 'youtube_oauth' } : {}),
