...
export function integrationStatus(env) {
  ...
  }), { id: 'youtube', platform: 'YouTube', ready: Boolean(env.YOUTUBE_API_KEY), missing: env.YOUTUBE_API_KEY ? [] : ['YOUTUBE_API_KEY'], status: env.YOUTUBE_API_KEY ? 'Общий ключ задан — требуется проверка API' : 'Общий ключ не задан; доступен персональный ключ', callbackUrl: null, scopes: [], enableVariable: null },
...
const allowedOrigins = new Set(['https://api.instagram.com', 'https://graph.instagram.com', 'https://open.tiktokapis.com', 'https://id.vk.ru']);
