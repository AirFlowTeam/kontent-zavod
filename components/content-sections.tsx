...
export function MetricCards({ metrics }: { metrics: Metrics }) {
  const items = [
    { label: 'Креаторов с публикациями', value: formatNumber(metrics.creatorCount), note: 'по активным роликам', icon: UserRoundCheck },
  ...
  return (
    <article className={`relative overflow-hidden rounded-3xl p-6 ${isAI ? 'bg-[var(--ai-surface)] text-[var(--ai-foreground)]' : 'bg-[var(--ugc-surface)] text-[var(--ugc-foreground)]'} ${muted ? 'opacity-45 grayscale-[25%]' : ''}`}>
      ...
      <div className="relative">
        ...
        <p className={`mt-8 text-sm font-semibold ${isAI ? 'opacity-60' : 'text-white/65'}`}>{formatNumber(metrics.creatorCount)} с публикациями · {formatNumber(metrics.videoCount)} роликов</p>
          ...
          <div className="divide-y divide-border md:hidden">
            {rows.map((row) => (
              <button key={row.id} aria-label={`Открыть карточку креатора ${row.name}`} onClick={() => onCreator(row.id)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left hover:bg-muted/40">
                ...
                <div className="shrink-0 text-right"><p className="font-bold tabular-nums">{formatNumber(row.reach)}</p><p className="text-[11px] text-muted-foreground">охват · средний {row.videoCount ? formatNumber(row.average) : '—'}</p></div>
              </button>
            ))}
          </div>
        </>
          ...
          {producerRows.length === 0 ? <EmptyState title="Нет данных" description="В выборке пока нет публикаций." /> : <div className="divide-y divide-border border-t border-border">
            {producerRows.slice(0, 4).map((row) => (
              <button key={row.id} aria-label={`Открыть карточку продюсера ${row.name}`} onClick={() => onProducer(row.id)} className="grid w-full grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-4 px-5 py-3 text-left hover:bg-muted/40 sm:px-6">
                <div className="min-w-0"><p className="truncate text-sm font-bold">{row.name}</p><p className="text-[11px] text-muted-foreground">{formatNumber(row.creatorCount ?? 0)} с публикациями</p></div>
...
export function CreatorTypeSection({ type, metrics, rows, onCreator }: { type: CreatorType; metrics: Metrics; rows: SummaryRow[]; onCreator: (id: number) => void }) {
  return <div className="space-y-5"><MetricCards metrics={metrics} /><CreatorTable rows={rows} onCreator={onCreator} title={`${type}-креаторы`} subtitle={`Все креаторы типа ${type}; метрики — только по активным публикациям`} /></div>;
}
      ...
      {videos.length === 0 ? <EmptyState title="За этот период роликов нет" description="Сбросьте фильтры или добавьте первую публикацию." action={<Button onClick={onAdd}>Добавить ролик</Button>} /> : (
        <>
          ...
          <div className="divide-y divide-border md:hidden">{videos.map((video) => <article key={video.id} className="px-5 py-4">
            <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="font-bold">{video.creatorName}</span><TypeBadge type={video.creatorType} /></div><p className="mt-1 text-xs text-muted-foreground">{formatDate(video.publishedAt)} · {video.platformName}</p><p className="mt-0.5 text-xs text-muted-foreground">Продюсер: {video.producerName}</p></div><Button variant="ghost" size="icon-sm" onClick={() => onEdit(video)} aria-label={`Редактировать ролик ${video.creatorName}`}><Pencil /></Button></div>
...
export function ProducersSection({ producers, creators, rows, onProducer, onEdit, onAdd }: { producers: Producer[]; creators: { producerId: number }[]; rows: SummaryRow[]; onProducer: (id: number) => void; onEdit: (producer: Producer) => void; onAdd: () => void }) {
    ...
    <article className="overflow-hidden rounded-3xl border border-border/80 bg-card">
      ...
      {producers.length === 0 ? <EmptyState title="Продюсеров пока нет" description="Добавьте первого продюсера, чтобы закреплять за ним креаторов." /> : <div className="grid gap-3 border-t border-border p-4 sm:grid-cols-2 xl:grid-cols-3 sm:p-5">{producers.map((producer) => {
        ...
        return <article key={producer.id} className="rounded-2xl border border-border bg-background/60 p-5 hover:border-primary/25">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4"><div><p className="text-xs text-muted-foreground">Всего закреплено</p><p className="mt-1 text-xl font-extrabold tabular-nums">{formatNumber(creators.filter((creator) => creator.producerId === producer.id).length)}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{formatNumber(row.creatorCount ?? 0)} с публикациями по выборке</p></div><div><p className="text-xs text-muted-foreground">Активных роликов</p><p className="mt-1 text-xl font-extrabold tabular-nums">{formatNumber(row.videoCount)}</p></div><div className="col-span-2 border-t border-border pt-4"><p className="text-xs text-muted-foreground">Общий охват</p><p className="mt-1 text-xl font-extrabold tabular-nums">{formatNumber(row.reach)}</p></div></div>
        </article>;
      })}</div>}
    </article>
  );
