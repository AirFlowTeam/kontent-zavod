...
export function CreatorDetailDialog({ creator, videos, onClose, onEdit, onEditVideo }: { creator: Creator | null; videos: Video[]; onClose: () => void; onEdit: (creator: Creator) => void; onEditVideo: (video: Video) => void }) {
  ...
  return <Dialog open={Boolean(creator)} onOpenChange={(next) => !next && onClose()}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-[720px]">{creator && <><DialogHeader><div className="flex items-start justify-between gap-8 pr-8"><div><div className="mb-2 flex items-center gap-2"><Badge className={creator.type === 'AI' ? 'bg-[var(--ai-soft)] text-[var(--ai-ink)]' : 'bg-[var(--ugc-soft)] text-[var(--ugc-ink)]'}>{creator.type}</Badge><Badge variant="outline">{creator.status === 'active' ? 'Активен' : 'Неактивен'}</Badge></div><DialogTitle className="text-2xl font-extrabold tracking-[-0.04em]">{creator.name}</DialogTitle><DialogDescription className="mt-1">Ответственный продюсер: {creator.producerName}</DialogDescription></div><Button variant="outline" size="sm" onClick={() => onEdit(creator)}><Pencil data-icon="inline-start" /> Редактировать</Button></div></DialogHeader><div className="grid grid-cols-1 gap-2 min-[390px]:grid-cols-2 sm:grid-cols-3">{[['Активных роликов', metrics.videoCount], ['Общий охват', metrics.reach], ['Средний', metrics.videoCount ? metrics.average : '—']].map(([label, value]) => <div key={label} className="min-w-0 rounded-2xl bg-muted/60 p-4"><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-1 truncate text-lg font-extrabold tabular-nums">{typeof value === 'number' ? formatNumber(value) : value}</p></div>)}</div><div><h3 className="mb-3 font-bold">Публикации</h3>{creatorVideos.length === 0 ? <p className="rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Публикаций пока нет.</p> : <div className="divide-y divide-border overflow-hidden rounded-2xl border border-border">{creatorVideos.map((video) => <div key={video.id} className="flex items-center justify-between gap-3 px-4 py-3"><div className="min-w-0"><p className="text-sm font-bold">{video.platformName} · {formatDate(video.publishedAt)}</p><div className="mt-1 flex items-center gap-2"><a href={video.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-primary">Открыть ролик <ExternalLink className="size-3" /></a>{video.status !== 'active' && <Badge variant="outline" className={video.status === 'error' ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-red-200 bg-red-50 text-red-700'}>{video.status === 'error' ? 'Ошибка' : 'Удалён'}</Badge>}</div></div><div className="flex items-center gap-3"><strong className="text-sm tabular-nums">{formatNumber(video.reach)}</strong><Button variant="ghost" size="icon-sm" aria-label={`Редактировать ролик ${video.platformName} от ${video.publishedAt}`} onClick={() => onEditVideo(video)}><Pencil /></Button></div></div>)}</div>}</div></>}</DialogContent></Dialog>;
}
...
export function ProducerDetailDialog({ producer, creators, rows, videos, onClose, onEdit, onCreator }: { producer: Producer | null; creators: Creator[]; rows: SummaryRow[]; videos: Video[]; onClose: () => void; onEdit: (producer: Producer) => void; onCreator: (id: number) => void }) {
  ...
  return <Dialog open={Boolean(producer)} onOpenChange={(next) => !next && onClose()}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-[680px]">{producer && <><DialogHeader><div className="flex items-start justify-between gap-8 pr-8"><div><Badge variant="outline" className="mb-2">{producer.status === 'active' ? 'Активен' : 'Неактивен'}</Badge><DialogTitle className="text-2xl font-extrabold tracking-[-0.04em]">{producer.name}</DialogTitle><DialogDescription className="mt-1">Результат закреплённых креаторов</DialogDescription></div><Button variant="outline" size="sm" onClick={() => onEdit(producer)}><Pencil data-icon="inline-start" /> Редактировать</Button></div></DialogHeader><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{[['Закреплено', producerCreators.length], ['Активных роликов', metrics.videoCount], ['Общий охват', metrics.reach], ['Средний', metrics.videoCount ? metrics.average : '—']].map(([label, value]) => <div key={label} className="rounded-2xl bg-muted/60 p-4"><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-1 text-lg font-extrabold tabular-nums">{typeof value === 'number' ? formatNumber(value) : value}</p></div>)}</div><div><h3 className="mb-3 font-bold">Креаторы</h3>{producerCreators.length === 0 ? <p className="rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Закреплённых креаторов пока нет.</p> : <div className="divide-y divide-border overflow-hidden rounded-2xl border border-border">{producerCreators.map((creator) => { const row = rowById.get(creator.id); return <button key={creator.id} aria-label={`Открыть карточку креатора ${creator.name}`} onClick={() => onCreator(creator.id)} className="flex w-full items-center justify-between px-4 py-3 text-left hover:bg-muted/40"><div><p className="text-sm font-bold">{creator.name}</p><p className="text-xs text-muted-foreground">{creator.type} · {creator.status === 'active' ? 'активен' : 'неактивен'}</p></div><div className="text-right"><p className="text-sm font-bold tabular-nums">{formatNumber(row?.reach ?? 0)}</p><p className="text-[10px] text-muted-foreground">{formatNumber(row?.videoCount ?? 0)} роликов</p></div></button>; })}</div>}</div></>}</DialogContent></Dialog>;
}
